...
layout(set = 0, binding = 0, std140) uniform AtmosphereUniforms {
...
vec3 computeScattering(vec3 rayStart, vec3 rayDir, float rayLength, vec3 sunDir) {
    ...
    for (int i = 0; i < ATMOSPHERE_SAMPLES; ++i) {
        ...
        if (sunRayLength > 0.0) {
            ...
            vec3 transmittance = exp(-atmosphere.rayleighBeta * (rayleighOpticalDepth + sunRayleighOpticalDepth) 
                                   - atmosphere.mieBeta * (mieOpticalDepth + sunMieOpticalDepth));
    ...
    mieScattering *= atmosphere.mieBeta * miePhase(cosTheta, atmosphere.mieG);
...
//
//    // ћасштаб от интенсивности солнца
...
//
//    // --- јтмосферное голубое рассе¤ние ---
//    float skyBlend = smoothstep(0.6, 0.85, sunDot);  // чем дальше от солнца Ч тем сильнее голубой оттенок
