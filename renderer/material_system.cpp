...
void MaterialSystem::CreateSceneDescriptorLayout() {
	std::vector<VkDescriptorSetLayoutBinding> bindings(2); // ИЗМЕНЕНО: теперь 2 binding

	// Binding 0: Scene UBO (как было)
	...
	VkDescriptorSetLayoutCreateInfo layoutInfo{};
	layoutInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
	layoutInfo.bindingCount = static_cast<uint32_t>(bindings.size()); // ИЗМЕНЕНО
	layoutInfo.pBindings = bindings.data(); // ИЗМЕНЕНО
